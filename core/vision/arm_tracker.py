...
Two-marker approach: ArUco ID=1 on the WRIST and ArUco ID=7 on the ELBOW
together define the arm axis. This is jitter-free (no PCA, no contour
dependency) and unambiguous (no flip-direction guessing).

Arm length still comes from the manually-entered DB value — the markers
only define the *direction*, then we extrapolate `arm_length_mm` along
that direction from the wrist.
...
class ArmTracker:
    ...
    Tracks the forearm using:
      1. ArUco ID=1 on the wrist  → s=0 anchor
      2. ArUco ID=7 on the elbow  → defines arm direction
      3. Arm length from DB       → defines the actual elbow point
                                    (= wrist + arm_dir × arm_length_mm)
      4. OpenCV contour detection → forearm width profile
    """
    ...
    def process(self, frame: np.ndarray, mat_state: dict) -> dict:
        """Run wrist + elbow detection + contour analysis."""
        ...
        if len(self._wrist_buffer) == 0 or len(self._elbow_buffer) == 0:
            self._ready = False
            return {
                "ready":      False,
                "wrist_fill": len(self._wrist_buffer),
        ...
        # Direction = (elbow_marker − wrist), normalized
        ...
        # Perpendicular direction (rotated 90° in image space)
        perp_dir = np.array([-arm_dir[1], arm_dir[0]], dtype=np.float32)
    ...
    def draw_overlay(self, frame: np.ndarray, arm_state: dict) -> np.ndarray:
        ...
        # Draw line directly between the two markers (wrist → elbow_marker)
        elbow_marker_px = arm_state.get("elbow_marker_px")
        ...
        if elbow_marker_px is not None:
            em = elbow_marker_px.astype(int)
            cv2.circle(frame, tuple(em), 10, (255, 100, 0), -1)
            cv2.circle(frame, tuple(em), 10, (255, 255, 255), 2)

        # Raw elbow marker position (small magenta dot, distinguishable from
        # the orange s=1 point) — useful to confirm the marker is detected.
        if elbow_marker_px is not None:
